...
inline std::vector<int> SelectByGrid(const std::vector<int> &indices,
                                     ...
                                     const cv::Size &size,
                                     const int limit,
                                     const bool staticPool = true,
                                     const SemanticConfig *config = nullptr)
{
    ...
    for(std::vector<int> &cell : cells)
        std::sort(cell.begin(), cell.end(), [&keys, staticPool, config](int a, int b) {
            const float wa = (config && config->enableSoftSelection) ?
                (staticPool ? config->staticFeatureWeight : config->fallbackFeatureWeight) : 1.0f;
            const float wb = wa;
            return keys[a].response * wa > keys[b].response * wb;
        });
                                     ...
                                     std::vector<float> *weights = nullptr)
{
    ...
    std::vector<int> selected = allowFallback
        ? SelectByGrid(staticIndices, keys, maskSize, config.targetFeatures, true, &config)
        : staticIndices;
       ...
       selected.size() < static_cast<size_t>(config.targetFeatures))
    {
        ...
        const std::vector<int> fallback = SelectByGrid(unknownIndices, keys, maskSize,
            std::min(deficit, fallbackLimit), false, &config);
    ...
    if(weights) { weights->clear(); weights->reserve(selected.size()); }
    for(int index : selected)
    {
        filteredKeys.push_back(keys[index]);
        filteredDescriptors.push_back(descriptors.row(index));
        const bool isStatic = std::find(staticIndices.begin(), staticIndices.end(), index) != staticIndices.end();
        sources.push_back(isStatic ? 0 : 1);
        if(weights) weights->push_back(config.enableSoftSelection ?
            std::max(config.minFeatureWeight, isStatic ? config.staticFeatureWeight : config.fallbackFeatureWeight) : 1.0f);
    }
