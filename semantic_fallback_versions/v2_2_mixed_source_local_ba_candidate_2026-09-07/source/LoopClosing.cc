                                             ...
                                             int &nNumCoincidences, std::vector<MapPoint*> &vpMPs, std::vector<MapPoint*> &vpMatchedMPs)
{
    ...
    for(KeyFrame* pKFi : vpBowCand)
    {
        if(!pKFi || pKFi->isBad())
            continue;
        if(mpTracker && mpTracker->GetSemanticConfig().enableProvisionalLoopSupport)
            ++mnSemanticLoopCandidates;
        ...
        for(int j=0; j<vpCovKFi.size(); ++j)
        {
            for(int k=0; k < vvpMatchedMPs[j].size(); ++k)
            {
                ...
                const bool provisionalAllowed = mpTracker && mpTracker->GetSemanticConfig().enableProvisionalLoopSupport &&
                    pMPi_j && pMPi_j->IsLoopEligible(mpTracker->GetSemanticConfig().loopConfig(), mpCurrentKF ? mpCurrentKF->mnId : 0);
                if(!pMPi_j || pMPi_j->isBad() || (pMPi_j->IsProvisional() && !provisionalAllowed))
                    continue;
        ...
        if(mpTracker && mpTracker->GetSemanticConfig().enableProvisionalLoopSupport)
            mnSemanticLoopMatches += static_cast<unsigned long>(numBoWMatches);
        ...
        if(numBoWMatches >= nBoWMatches) // TODO pick a good threshold
        {
            ...
            int provisionalMatches = 0;
            for(MapPoint* pMP : vpMatchedPoints)
            {
                if(!pMP) continue;
                if(pMP->IsProvisional()) ++provisionalMatches;
                else ++trustedMatches;
            }
            const bool loopSupportEnabled = mpTracker && mpTracker->GetSemanticConfig().enableProvisionalLoopSupport;
            if(loopSupportEnabled && trustedMatches < mpTracker->GetSemanticConfig().provisionalLoopMinTrustedInliers)
                continue;
            if(loopSupportEnabled && provisionalMatches > 0 &&
               static_cast<float>(provisionalMatches) / static_cast<float>(trustedMatches + provisionalMatches) >
               mpTracker->GetSemanticConfig().provisionalLoopMaxFraction)
                continue;
            ...
            if(bConverge)
            {
                int trustedInliers = 0;
                int provisionalInliers = 0;
                for(size_t inlierIndex = 0; inlierIndex < vbInliers.size() && inlierIndex < vpMatchedPoints.size(); ++inlierIndex)
                {
                    if(!vbInliers[inlierIndex] || !vpMatchedPoints[inlierIndex]) continue;
                    if(vpMatchedPoints[inlierIndex]->IsProvisional()) ++provisionalInliers;
                    else ++trustedInliers;
                }
                if(mpTracker && mpTracker->GetSemanticConfig().enableProvisionalLoopSupport)
                {
                    mnSemanticLoopTrustedInliers += static_cast<unsigned long>(trustedInliers);
                    mnSemanticLoopProvisionalInliers += static_cast<unsigned long>(provisionalInliers);
                }
                const bool loopSupportEnabled = mpTracker && mpTracker->GetSemanticConfig().enableProvisionalLoopSupport;
                if(loopSupportEnabled && trustedInliers < mpTracker->GetSemanticConfig().provisionalLoopMinTrustedInliers)
                    continue;
                if(loopSupportEnabled && provisionalInliers > 0 &&
                   static_cast<float>(provisionalInliers) / static_cast<float>(trustedInliers + provisionalInliers) >
                   mpTracker->GetSemanticConfig().provisionalLoopMaxFraction)
                    continue;
    ...
    for(KeyFrame* pKFi : vpCovKFm)
    {
        for(MapPoint* pMPij : pKFi->GetMapPointMatches())
        {
            if(!pMPij || pMPij->isBad())
                continue;
            if(pMPij->IsProvisional())
            {
                const bool provisionalAllowed = mpTracker && mpTracker->GetSemanticConfig().enableProvisionalLoopSupport &&
                    pMPij->IsLoopEligible(mpTracker->GetSemanticConfig().loopConfig(), pCurrentKF ? pCurrentKF->mnId : 0);
