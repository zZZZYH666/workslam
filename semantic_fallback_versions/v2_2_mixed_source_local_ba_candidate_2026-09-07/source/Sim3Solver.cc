...
void Sim3Solver::ComputeSim3(Eigen::Matrix3f &P1, Eigen::Matrix3f &P2)
{
    ...
    if(!std::isfinite(vecNorm) || vecNorm < 1e-7f || !std::isfinite(evec(0,maxIndex)))
    {
        mR12i.setIdentity();
    }
    else
    ...
    if(!mbFixScale)
    {
        ...
        ms12i = (std::isfinite(nom) && std::isfinite(den) && std::abs(den) > 1e-9) ? static_cast<float>(nom/den) : 1.0f;
        if(!std::isfinite(ms12i) || std::abs(ms12i) < 1e-6f)
            ms12i = 1.0f;
    }
    else
        ms12i = 1.0f;
