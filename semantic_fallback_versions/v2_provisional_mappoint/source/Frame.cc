...
Frame::Frame(const Frame &frame)
    :mpcpi(frame.mpcpi),mpORBvocabulary(frame.mpORBvocabulary), mpORBextractorLeft(frame.mpORBextractorLeft), mpORBextractorRight(frame.mpORBextractorRight),
     ...
     mbf(frame.mbf), mb(frame.mb), mThDepth(frame.mThDepth), N(frame.N), mnRawLeftFeatures(frame.mnRawLeftFeatures), mnRawRightFeatures(frame.mnRawRightFeatures), mvKeys(frame.mvKeys),
     mvKeysRight(frame.mvKeysRight), mvKeysUn(frame.mvKeysUn), mvSemanticSource(frame.mvSemanticSource), mvSemanticSourceRight(frame.mvSemanticSourceRight), mvSemanticWeight(frame.mvSemanticWeight), mvSemanticWeightRight(frame.mvSemanticWeightRight), mvuRight(frame.mvuRight),
     ...
     mpCamera(pCamera) ,mpCamera2(nullptr), mbHasPose(false), mbHasVelocity(false)
{
    ...
    mnRawLeftFeatures = static_cast<int>(mvKeys.size());
    mnRawRightFeatures = static_cast<int>(mvKeysRight.size());
    const std::vector<cv::KeyPoint> rawLeftKeys = mvKeys;
    ...
    if(SemanticFallback::ShouldRetryForStereo(semanticConfig, fallbackRequested, mnStereoMatches))
    {
        ...
        mnFallbackLeftFeatures = left.fallbackCount;
        mnFallbackRightFeatures = right.fallbackCount;
        mbSemanticFallback = mnFallbackLeftFeatures > 0 || mnFallbackRightFeatures > 0;
