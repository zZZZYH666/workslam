...
KeyFrame::KeyFrame(Frame &F, Map *pMap, KeyFrameDatabase *pKFDB):
    ...
    mvSemanticSource(F.mvSemanticSource), mvSemanticWeight(F.mvSemanticWeight),
    ...
    mvKeysRight(F.mvKeysRight), mvSemanticSourceRight(F.mvSemanticSourceRight), mvSemanticWeightRight(F.mvSemanticWeightRight), NLeft(F.Nleft), NRight(F.Nright), mTrl(F.GetRelativePoseTrl()), mnNumberOfOpt(0), mbHasVelocity(false)
{
    mnId=nNextId++;
