        ...
        if(pMP)
        {
                ...
                if (pMP->IsProvisional())
                {
                    const bool staticObservation =
                        (i >= mpCurrentKeyFrame->mvSemanticSource.size()) ||
                        !mpCurrentKeyFrame->mvSemanticSource[i];
...
void LocalMapping::CreateNewMapPoints()
{
    ...
    for(size_t i=0; i<vpNeighKFs.size(); i++)
    {
        ...
        for(int ikp=0; ikp<nmatches; ikp++)
        {
            ...
            const bool fallbackSource =
                ((idx1 >= 0 && idx1 < static_cast<int>(mpCurrentKeyFrame->mvSemanticSource.size())) && mpCurrentKeyFrame->mvSemanticSource[idx1]) ||
                ((idx2 >= 0 && idx2 < static_cast<int>(pKF2->mvSemanticSource.size())) && pKF2->mvSemanticSource[idx2]);
            if(mpTracker && mpTracker->GetSemanticConfig().enableMapPointPromotion && fallbackSource)
            {
                ...
                pMP->RegisterSemanticMatch(false, 0.0f);
                pMP->RegisterSemanticKeyFrameObservation(mpCurrentKeyFrame->mnId, false);
                pMP->RegisterSemanticKeyFrameObservation(pKF2->mnId, false);
            }
            if (bPointStereo)
                countStereo++;
