    ...
    if(mCurrentFrame.N>500)
    {
        ...
        auto initializeSemanticMapPoint = [&](MapPoint* pMP, int leftIndex, int rightIndex = -1) {
            ...
            if (leftFallback || rightFallback) {
                ...
                pMP->RegisterSemanticVisibility();
                pMP->RegisterSemanticMatch(!(leftFallback || rightFallback), 0.0f);
                pMP->RegisterSemanticKeyFrameObservation(pKFini->mnId, !(leftFallback || rightFallback));
            }
        };
...
bool Tracking::TrackLocalMap()
{
    ...
    for(int i=0; i<mCurrentFrame.N; i++)
    {
        if(mCurrentFrame.mvpMapPoints[i])
        {
            MapPoint* pSemanticMP = mCurrentFrame.mvpMapPoints[i];
            if (pSemanticMP->IsProvisional())
            {
                pSemanticMP->RegisterSemanticVisibility();
                const bool staticObservation =
                    (i >= static_cast<int>(mCurrentFrame.mvSemanticSource.size())) ||
                    !mCurrentFrame.mvSemanticSource[i];
                if (!mCurrentFrame.mvbOutlier[i])
                    pSemanticMP->RegisterSemanticMatch(staticObservation, 0.0f);
            }
            if(!mCurrentFrame.mvbOutlier[i])
            {
                mCurrentFrame.mvpMapPoints[i]->IncreaseFound();
                ...
                if(bCreateNew)
                {
                    ...
                    if (mSemanticConfig.enableMapPointPromotion)
                    {
                        ...
                        if (leftFallback || rightFallback)
                        {
                            ...
                            pNewMP->RegisterSemanticVisibility();
                            pNewMP->RegisterSemanticMatch(false, 0.0f);
                            pNewMP->RegisterSemanticKeyFrameObservation(pKF->mnId, false);
                        }
                    }
...
bool Tracking::Relocalization()
{
    ...
    for(int i=0; i<nKFs; i++)
    {
        ...
        else
        {
            ...
            {
                // Provisional points remain eligible for short-term relocalization;
                // they are still excluded from loop closing and global BA.
            }
            if(nmatches<15)
            {
                vbDiscarded[i] = true;
