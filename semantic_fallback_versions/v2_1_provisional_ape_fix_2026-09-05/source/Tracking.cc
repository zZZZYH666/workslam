...
namespace
{
bool ComputeSemanticReprojectionError(Frame &frame, int index, MapPoint *point, float &error)
{
    if(!point || !frame.HasPose())
        return false;

    const bool right = frame.Nleft != -1 && index >= frame.Nleft;
    int localIndex = index;
    Sophus::SE3f pose = frame.GetPose();
    GeometricCamera *camera = frame.mpCamera;
    cv::Point2f observed;
    if(right)
    {
        localIndex -= frame.Nleft;
        if(localIndex < 0 || localIndex >= static_cast<int>(frame.mvKeysRight.size()) || !frame.mpCamera2)
            return false;
        pose = frame.GetRelativePoseTrl() * pose;
        camera = frame.mpCamera2;
        observed = frame.mvKeysRight[localIndex].pt;
    }
    else
    {
        if(localIndex < 0 || localIndex >= static_cast<int>(frame.mvKeysUn.size()))
            return false;
        observed = frame.mvKeysUn[localIndex].pt;
    }

    const Eigen::Vector3f cameraPoint = pose * point->GetWorldPos();
    if(cameraPoint(2) <= 0.0f)
        return false;
    const Eigen::Vector2f projected = camera->project(cameraPoint);
    if(!std::isfinite(projected(0)) || !std::isfinite(projected(1)))
        return false;
    const float dx = projected(0) - observed.x;
    const float dy = projected(1) - observed.y;
    error = std::sqrt(dx * dx + dy * dy);
    return std::isfinite(error);
}

bool SemanticSourceIsStatic(const Frame &frame, int index)
{
    if(frame.Nleft != -1 && index >= frame.Nleft)
    {
        const int rightIndex = index - frame.Nleft;
        return rightIndex < 0 || rightIndex >= static_cast<int>(frame.mvSemanticSourceRight.size()) ||
               frame.mvSemanticSourceRight[rightIndex] == 0;
    }
    return index < 0 || index >= static_cast<int>(frame.mvSemanticSource.size()) || frame.mvSemanticSource[index] == 0;
}
}
...
Tracking::Tracking(System *pSys, ORBVocabulary* pVoc, FrameDrawer *pFrameDrawer, MapDrawer *pMapDrawer, Atlas *pAtlas, KeyFrameDatabase* pKFDB, const string &strSettingPath, const int sensor, Settings* settings, const string &_nameSeq):
    ...
    mnInitialFrameId(0), mbCreatedMap(false), mnFirstFrameId(0), mpCamera2(nullptr), mpLastKeyFrame(static_cast<KeyFrame*>(NULL)), mnMatchesInliers(0)
{
    // Load camera parameters from settings file
    if(settings){
        newParameterLoader(settings);
        mSemanticConfig = settings->semanticConfig();
    }
    else{
                                      ...
                                      const cv::Mat &maskLeft, const cv::Mat &maskRight)
{
    ...
    Track();
    mCurrentFrame.mnTrackingInliers = mnMatchesInliers;
    ...
    if(mCurrentFrame.N>500)
    {
        ...
        auto initializeSemanticMapPoint = [&](MapPoint* pMP, int leftIndex, int rightIndex = -1) {
            ...
            if (leftFallback || rightFallback) {
                ...
                if(leftIndex >= 0)
                    reprojectionValid = ComputeSemanticReprojectionError(mCurrentFrame, leftIndex, pMP, reprojectionError);
                if(rightIndex >= 0)
                {
                    float rightError = 0.0f;
                    const bool rightValid = ComputeSemanticReprojectionError(mCurrentFrame, mCurrentFrame.Nleft + rightIndex, pMP, rightError);
                    if(rightValid && (!reprojectionValid || rightError < reprojectionError))
                    {
                        reprojectionError = rightError;
        ...
        if(!mpCamera2){
            for(int i=0; i<mCurrentFrame.N;i++)
            {
                ...
                if(z>0)
                {
                    ...
                    initializeSemanticMapPoint(pNewMP, i);
            ...
            for(int i = 0; i < mCurrentFrame.Nleft; i++){
                ...
                if(rightIndex != -1){
                    ...
                    MapPoint* pNewMP = new MapPoint(x3D, pKFini, mpAtlas->GetCurrentMap());
                    initializeSemanticMapPoint(pNewMP, i, rightIndex);
...
bool Tracking::TrackLocalMap()
{
    ...
    // Update MapPoints Statistics. Aggregate stereo observations first so a
    // static observation in either eye is retained and each point is counted
    // at most once per frame.
    struct SemanticFrameObservation
    {
        bool staticObservation = false;
        bool matched = false;
        bool reprojectionValid = false;
        float reprojectionError = 0.0f;
    };
    std::map<MapPoint*, SemanticFrameObservation> semanticObservations;
    for(int i=0; i<mCurrentFrame.N; i++)
    {
        MapPoint* pMP = mCurrentFrame.mvpMapPoints[i];
        if(!pMP || !pMP->IsProvisional())
            continue;
        SemanticFrameObservation &observation = semanticObservations[pMP];
        observation.staticObservation = observation.staticObservation || SemanticSourceIsStatic(mCurrentFrame, i);
        if(!mCurrentFrame.mvbOutlier[i])
        {
            observation.matched = true;
            float reprojectionError = 0.0f;
            if(ComputeSemanticReprojectionError(mCurrentFrame, i, pMP, reprojectionError) &&
               (!observation.reprojectionValid || reprojectionError < observation.reprojectionError))
            {
                observation.reprojectionError = reprojectionError;
                observation.reprojectionValid = true;
            }
        }
    }
    for(const auto &entry : semanticObservations)
    {
                ...
                if(bCreateNew)
                {
                    ...
                    if (mSemanticConfig.enableMapPointPromotion)
                    {
                        ...
                        if (leftFallback || rightFallback)
                        {
                            ...
                            {
                                float rightError = 0.0f;
                                const bool rightValid = ComputeSemanticReprojectionError(mCurrentFrame, mCurrentFrame.Nleft + rightIndex, pNewMP, rightError);
                                if(rightValid && (!reprojectionValid || rightError < reprojectionError))
                                {
                                    reprojectionError = rightError;
                                    reprojectionValid = true;
...
bool Tracking::Relocalization()
{
    ...
    for(int i=0; i<nKFs; i++)
    {
        ...
        else
        {
            ...
            for (MapPoint*& pMatch : vvpMapPointMatches[i])
            {
                if(pMatch && pMatch->IsProvisional())
                {
                    const bool provisionalAllowed = mSemanticConfig.enableProvisionalLoopSupport &&
                        pMatch->IsLoopEligible(mSemanticConfig.loopConfig(), mpLastKeyFrame ? mpLastKeyFrame->mnId : 0);
                    if(!provisionalAllowed)
                    {
                        pMatch = static_cast<MapPoint*>(NULL);
                        continue;
                    }
                }
                if(pMatch) ++acceptedMatches;
            }
            nmatches = acceptedMatches;
            if(nmatches<15)
            {
                vbDiscarded[i] = true;
                continue;
