...
{
    return TrackStereo(imLeft, imRight, timestamp, cv::Mat(), cv::Mat(), vImuMeas, filename);
}
                                ...
                                const vector<IMU::Point>& vImuMeas, string filename)
{
    ...
    const bool useMasks = !maskLeft.empty() || !maskRight.empty();
    if(useMasks && (maskLeft.empty() || maskRight.empty() || maskLeft.type() != CV_8UC1 || maskRight.type() != CV_8UC1 ||
                    maskLeft.size() != imLeft.size() || maskRight.size() != imRight.size()))
    {
        throw std::invalid_argument("Stereo semantic masks must both be CV_8UC1 and match their input images");
    }
...
void System::SaveTrajectoryEuRoC(const string &filename)
{
    ...
    if(!pBiggerMap)
    {
        ...
        emptyFile.close();
        cout << "There is not a map with keyframes." << endl;
        return;
    }
...
int System::GetSemanticStaticFeatures() const
{
    return mpTracker ? mpTracker->mCurrentFrame.mnStaticLeftFeatures : 0;
}

int System::GetSemanticRawLeftFeatures() const
{
    return mpTracker ? mpTracker->mCurrentFrame.mnRawLeftFeatures : 0;
}

int System::GetSemanticRawRightFeatures() const
{
    return mpTracker ? mpTracker->mCurrentFrame.mnRawRightFeatures : 0;
}

int System::GetSemanticStaticRightFeatures() const
{
    return mpTracker ? mpTracker->mCurrentFrame.mnStaticRightFeatures : 0;
}

int System::GetSemanticFallbackFeatures() const
{
    return mpTracker ? mpTracker->mCurrentFrame.mnFallbackLeftFeatures : 0;
}

int System::GetSemanticFallbackRightFeatures() const
{
    return mpTracker ? mpTracker->mCurrentFrame.mnFallbackRightFeatures : 0;
}

int System::GetSemanticStereoMatches() const
{
    return mpTracker ? mpTracker->mCurrentFrame.mnStereoMatches : 0;
}

float System::GetSemanticGridCoverage() const
{
    return mpTracker ? mpTracker->mCurrentFrame.mfLeftGridCoverage : 0.0f;
}

bool System::SemanticFallbackUsed() const
{
    return mpTracker && mpTracker->mCurrentFrame.mbSemanticFallback;
}

int System::GetSemanticFallbackReason() const
{
...
long unsigned int System::GetAtlasKeyFrames()
{
    return mpAtlas ? mpAtlas->KeyFramesInMap() : 0;
}

long unsigned int System::GetAtlasMapPoints()
{
    return mpAtlas ? mpAtlas->MapPointsInMap() : 0;
}

long unsigned int System::GetSemanticStaticMapPoints()
{
    long unsigned int n=0; if(!mpAtlas) return 0;
    for(MapPoint* p : mpAtlas->GetAllMapPoints()) if(p && !p->isBad() && p->GetSemanticState()==SemanticMapPointState::STATIC) ++n; return n;
}
long unsigned int System::GetSemanticProvisionalMapPoints()
{
    long unsigned int n=0; if(!mpAtlas) return 0;
    for(MapPoint* p : mpAtlas->GetAllMapPoints()) if(p && !p->isBad() && p->IsProvisional()) ++n; return n;
}
long unsigned int System::GetSemanticPromotedMapPoints()
{
    long unsigned int n=0; if(!mpAtlas) return 0;
    for(MapPoint* p : mpAtlas->GetAllMapPoints()) if(p && !p->isBad() && p->IsPromoted()) ++n; return n;
}
long unsigned int System::GetSemanticRejectedMapPoints() { return MapPoint::nSemanticRejected; }
long unsigned int System::GetSemanticProvisionalVisible()
{
    long unsigned int n=0; if(!mpAtlas) return 0;
    for(MapPoint* p : mpAtlas->GetAllMapPoints()) if(p && p->IsProvisional()) n += p->SemanticVisibleFrames(); return n;
}
long unsigned int System::GetSemanticProvisionalMatched()
{
    long unsigned int n=0; if(!mpAtlas) return 0;
    for(MapPoint* p : mpAtlas->GetAllMapPoints()) if(p && p->IsProvisional()) n += p->SemanticMatchedFrames(); return n;
}
long unsigned int System::GetSemanticPromotionCount() { return MapPoint::nSemanticPromoted; }
long unsigned int System::GetSemanticRejectionCount() { return MapPoint::nSemanticRejected; }
std::vector<MapPoint::SemanticLifecycleEvent> System::GetSemanticLifecycleEvents() const
{
    return MapPoint::GetSemanticLifecycleEvents();
}
std::vector<MapPoint::SemanticLifecycleEvent> System::GetSemanticLifecycleEventsSince(size_t startIndex) const
{
    return MapPoint::GetSemanticLifecycleEventsSince(startIndex);
}
void System::ClearSemanticLifecycleEvents()
{
    MapPoint::ClearSemanticLifecycleEvents();
}
int System::GetTrackingInliers() const
{
    return mpTracker ? mpTracker->GetTrackingInliers() : 0;
}
size_t System::GetLocalMapPointCount() const
{
    return mpTracker ? mpTracker->GetLocalMapPointCount() : 0;
}
unsigned long System::GetLastKeyFrameId() const
{
    return mpTracker ? mpTracker->GetLastKeyFrameId() : 0;
}
unsigned long System::GetSemanticLoopCandidates() const
{
    return mpLoopCloser ? mpLoopCloser->GetSemanticLoopCandidates() : 0;
}
unsigned long System::GetSemanticLoopMatches() const
{
    return mpLoopCloser ? mpLoopCloser->GetSemanticLoopMatches() : 0;
}
unsigned long System::GetSemanticLoopTrustedInliers() const
{
    return mpLoopCloser ? mpLoopCloser->GetSemanticLoopTrustedInliers() : 0;
}
unsigned long System::GetSemanticLoopProvisionalInliers() const
{
    return mpLoopCloser ? mpLoopCloser->GetSemanticLoopProvisionalInliers() : 0;
}
unsigned long System::GetCurrentMapId() const
{
    return (mpAtlas && mpAtlas->GetCurrentMap()) ? mpAtlas->GetCurrentMap()->GetId() : 0;
}
