...
namespace ORB_SLAM3 {
    ...
    void Settings::readCamera2(cv::FileStorage &fSettings) {
        ...
        if (cameraType_ == PinHole) {
            ...
            // Rectified stereo inputs use projection intrinsics directly.
            // No distortion model or remapping is applied in this mode.
            float fx = readParameter<float>(fSettings,"Camera2.fx",found);
            float fy = readParameter<float>(fSettings,"Camera2.fy",found);
            float cx = readParameter<float>(fSettings,"Camera2.cx",found);
            float cy = readParameter<float>(fSettings,"Camera2.cy",found);
    ...
    void Settings::readSemantic(cv::FileStorage& fSettings) {
        bool found;
        const int enabled = readParameter<int>(fSettings,"Semantic.EnableFallback",found,false);
        semanticConfig_.enableFallback = found && enabled != 0;
        int intValue = readParameter<int>(fSettings,"Semantic.TargetFeatures",found,false);
        if(found) semanticConfig_.targetFeatures = intValue;
        intValue = readParameter<int>(fSettings,"Semantic.MinStaticFeatures",found,false);
        if(found) semanticConfig_.minStaticFeatures = intValue;
        intValue = readParameter<int>(fSettings,"Semantic.MinStereoMatches",found,false);
        if(found) semanticConfig_.minStereoMatches = intValue;
        intValue = readParameter<int>(fSettings,"Semantic.MinTrackingInliers",found,false);
        if(found) semanticConfig_.minTrackingInliers = intValue;
        float floatValue = readParameter<float>(fSettings,"Semantic.MinGridCoverage",found,false);
        if(found) semanticConfig_.minGridCoverage = floatValue;
        floatValue = readParameter<float>(fSettings,"Semantic.MaxFallbackRatio",found,false);
        if(found) semanticConfig_.maxFallbackRatio = floatValue;
        const int promotionEnabled = readParameter<int>(fSettings,"Semantic.EnableMapPointPromotion",found,false);
        if(found) semanticConfig_.enableMapPointPromotion = promotionEnabled != 0;
        intValue = readParameter<int>(fSettings,"Semantic.PromotionMinVisibleFrames",found,false);
        if(found) semanticConfig_.promotionMinVisibleFrames = intValue;
        intValue = readParameter<int>(fSettings,"Semantic.PromotionMinKeyFrames",found,false);
        if(found) semanticConfig_.promotionMinKeyFrames = intValue;
        floatValue = readParameter<float>(fSettings,"Semantic.PromotionMinMatchRatio",found,false);
        if(found) semanticConfig_.promotionMinMatchRatio = floatValue;
        floatValue = readParameter<float>(fSettings,"Semantic.PromotionMaxReprojectionError",found,false);
        if(found) semanticConfig_.promotionMaxReprojectionError = floatValue;
        floatValue = readParameter<float>(fSettings,"Semantic.PromotionMinStaticRatio",found,false);
        if(found) semanticConfig_.promotionMinStaticRatio = floatValue;
        intValue = readParameter<int>(fSettings,"Semantic.ProvisionalMaxAgeKeyFrames",found,false);
        if(found) semanticConfig_.provisionalMaxAgeKeyFrames = intValue;
        intValue = readParameter<int>(fSettings,"Semantic.ProvisionalMaxDynamicObservations",found,false);
        if(found) semanticConfig_.provisionalMaxDynamicObservations = intValue;
        const int geometricPromotionEnabled = readParameter<int>(fSettings,"Semantic.EnableGeometricPromotion",found,false);
        if(found) semanticConfig_.enableGeometricPromotion = geometricPromotionEnabled != 0;
        intValue = readParameter<int>(fSettings,"Semantic.GeometricPromotionMinVisibleFrames",found,false);
        if(found) semanticConfig_.geometricPromotionMinVisibleFrames = intValue;
        intValue = readParameter<int>(fSettings,"Semantic.GeometricPromotionMinKeyFrames",found,false);
        if(found) semanticConfig_.geometricPromotionMinKeyFrames = intValue;
        floatValue = readParameter<float>(fSettings,"Semantic.GeometricPromotionMinMatchRatio",found,false);
        if(found) semanticConfig_.geometricPromotionMinMatchRatio = floatValue;
        floatValue = readParameter<float>(fSettings,"Semantic.GeometricPromotionMaxReprojectionError",found,false);
        if(found) semanticConfig_.geometricPromotionMaxReprojectionError = floatValue;
        intValue = readParameter<int>(fSettings,"Semantic.GeometricPromotionMaxDynamicObservations",found,false);
        if(found) semanticConfig_.geometricPromotionMaxDynamicObservations = intValue;
        const int loopSupportEnabled = readParameter<int>(fSettings,"Semantic.EnableProvisionalLoopSupport",found,false);
        if(found) semanticConfig_.enableProvisionalLoopSupport = loopSupportEnabled != 0;
        intValue = readParameter<int>(fSettings,"Semantic.ProvisionalLoopMinVisibleFrames",found,false);
        if(found) semanticConfig_.provisionalLoopMinVisibleFrames = intValue;
        intValue = readParameter<int>(fSettings,"Semantic.ProvisionalLoopMinKeyFrames",found,false);
        if(found) semanticConfig_.provisionalLoopMinKeyFrames = intValue;
        floatValue = readParameter<float>(fSettings,"Semantic.ProvisionalLoopMinMatchRatio",found,false);
        if(found) semanticConfig_.provisionalLoopMinMatchRatio = floatValue;
        intValue = readParameter<int>(fSettings,"Semantic.ProvisionalLoopMaxAgeKeyFrames",found,false);
        if(found) semanticConfig_.provisionalLoopMaxAgeKeyFrames = intValue;
        intValue = readParameter<int>(fSettings,"Semantic.ProvisionalLoopMaxDynamicObservations",found,false);
        if(found) semanticConfig_.provisionalLoopMaxDynamicObservations = intValue;
        intValue = readParameter<int>(fSettings,"Semantic.ProvisionalLoopMinTrustedInliers",found,false);
        if(found) semanticConfig_.provisionalLoopMinTrustedInliers = intValue;
        floatValue = readParameter<float>(fSettings,"Semantic.ProvisionalLoopMaxFraction",found,false);
        if(found) semanticConfig_.provisionalLoopMaxFraction = floatValue;
        const int softSelectionEnabled = readParameter<int>(fSettings,"Semantic.EnableSoftSelection",found,false);
        if(found) semanticConfig_.enableSoftSelection = softSelectionEnabled != 0;
        floatValue = readParameter<float>(fSettings,"Semantic.StaticFeatureWeight",found,false);
        if(found) semanticConfig_.staticFeatureWeight = floatValue;
        floatValue = readParameter<float>(fSettings,"Semantic.FallbackFeatureWeight",found,false);
        ...
        semanticConfig_.minTrackingInliers = std::max(0, semanticConfig_.minTrackingInliers);
        semanticConfig_.minGridCoverage = std::max(0.0f, std::min(1.0f, semanticConfig_.minGridCoverage));
        semanticConfig_.maxFallbackRatio = std::max(0.0f, std::min(1.0f, semanticConfig_.maxFallbackRatio));
        semanticConfig_.promotionMinVisibleFrames = std::max(1, semanticConfig_.promotionMinVisibleFrames);
        semanticConfig_.promotionMinKeyFrames = std::max(1, semanticConfig_.promotionMinKeyFrames);
        semanticConfig_.promotionMinMatchRatio = std::max(0.0f, std::min(1.0f, semanticConfig_.promotionMinMatchRatio));
        semanticConfig_.promotionMaxReprojectionError = std::max(0.0f, semanticConfig_.promotionMaxReprojectionError);
        semanticConfig_.promotionMinStaticRatio = std::max(0.0f, std::min(1.0f, semanticConfig_.promotionMinStaticRatio));
        semanticConfig_.provisionalMaxAgeKeyFrames = std::max(1, semanticConfig_.provisionalMaxAgeKeyFrames);
        semanticConfig_.provisionalMaxDynamicObservations = std::max(0, semanticConfig_.provisionalMaxDynamicObservations);
        semanticConfig_.geometricPromotionMinVisibleFrames = std::max(1, semanticConfig_.geometricPromotionMinVisibleFrames);
        semanticConfig_.geometricPromotionMinKeyFrames = std::max(1, semanticConfig_.geometricPromotionMinKeyFrames);
        semanticConfig_.geometricPromotionMinMatchRatio = std::max(0.0f, std::min(1.0f, semanticConfig_.geometricPromotionMinMatchRatio));
        semanticConfig_.geometricPromotionMaxReprojectionError = std::max(0.0f, semanticConfig_.geometricPromotionMaxReprojectionError);
        semanticConfig_.geometricPromotionMaxDynamicObservations = std::max(0, semanticConfig_.geometricPromotionMaxDynamicObservations);
        semanticConfig_.provisionalLoopMinVisibleFrames = std::max(1, semanticConfig_.provisionalLoopMinVisibleFrames);
        semanticConfig_.provisionalLoopMinKeyFrames = std::max(1, semanticConfig_.provisionalLoopMinKeyFrames);
        semanticConfig_.provisionalLoopMinMatchRatio = std::max(0.0f, std::min(1.0f, semanticConfig_.provisionalLoopMinMatchRatio));
        semanticConfig_.provisionalLoopMaxAgeKeyFrames = std::max(1, semanticConfig_.provisionalLoopMaxAgeKeyFrames);
        semanticConfig_.provisionalLoopMaxDynamicObservations = std::max(0, semanticConfig_.provisionalLoopMaxDynamicObservations);
        semanticConfig_.provisionalLoopMinTrustedInliers = std::max(1, semanticConfig_.provisionalLoopMinTrustedInliers);
        semanticConfig_.provisionalLoopMaxFraction = std::max(0.0f, std::min(1.0f, semanticConfig_.provisionalLoopMaxFraction));
        ...
        if(semanticConfig_.fallbackFeatureWeight < semanticConfig_.minFeatureWeight)
            semanticConfig_.fallbackFeatureWeight = semanticConfig_.minFeatureWeight;
    }
