...
namespace
{
bool KeyFrameSemanticSourceIsStatic(const KeyFrame *kf, int index)
{
    if(!kf) return false;
    if(kf->NLeft != -1 && index >= kf->NLeft)
    {
        const int rightIndex = index - kf->NLeft;
        return rightIndex < 0 || rightIndex >= static_cast<int>(kf->mvSemanticSourceRight.size()) ||
               kf->mvSemanticSourceRight[rightIndex] == 0;
    }
    return index < 0 || index >= static_cast<int>(kf->mvSemanticSource.size()) || kf->mvSemanticSource[index] == 0;
}

bool ComputeKeyFrameReprojectionError(KeyFrame *kf, int index, const Eigen::Vector3f &worldPoint, float &error)
{
    if(!kf) return false;
    const bool right = kf->NLeft != -1 && index >= kf->NLeft;
    Sophus::SE3f pose = kf->GetPose();
    GeometricCamera *camera = kf->mpCamera;
    cv::Point2f observed;
    int localIndex = index;
    if(right)
    {
        localIndex -= kf->NLeft;
        if(localIndex < 0 || localIndex >= static_cast<int>(kf->mvKeysRight.size()) || !kf->mpCamera2)
            return false;
        pose = kf->GetRightPose();
        camera = kf->mpCamera2;
        observed = kf->mvKeysRight[localIndex].pt;
    }
    else
    {
        if(localIndex < 0 || localIndex >= static_cast<int>(kf->mvKeysUn.size()))
            return false;
        observed = kf->mvKeysUn[localIndex].pt;
    }
    const Eigen::Vector3f cameraPoint = pose * worldPoint;
    if(cameraPoint(2) <= 0.0f) return false;
    const Eigen::Vector2f projected = camera->project(cameraPoint);
    if(!std::isfinite(projected(0)) || !std::isfinite(projected(1))) return false;
...
void LocalMapping::MapPointCulling()
{
    ...
    while(lit!=mlpRecentAddedMapPoints.end())
    {
        ...
        {
            const bool semanticPromotion = pMP->CanPromote(mpTracker->GetSemanticConfig().promotionConfig());
            pMP->Promote(semanticPromotion ? "promotion_semantic" : "promotion_geometric");
            lit = mlpRecentAddedMapPoints.erase(lit);
        }
        else if(mpTracker && mpTracker->GetSemanticConfig().enableMapPointPromotion && pMP->IsProvisional() &&
        ...
        else if(!pMP->IsProvisional() && pMP->GetFoundRatio()<0.25f)
        {
            pMP->SetBadFlag();
            lit = mlpRecentAddedMapPoints.erase(lit);
        }
        else if(!pMP->IsProvisional() && ((int)nCurrentKFid-(int)pMP->mnFirstKFid)>=2 && pMP->Observations()<=cnThObs)
        {
            pMP->SetBadFlag();
            lit = mlpRecentAddedMapPoints.erase(lit);
        }
        else if(!pMP->IsProvisional() && ((int)nCurrentKFid-(int)pMP->mnFirstKFid)>=3)
            lit = mlpRecentAddedMapPoints.erase(lit);
        else
        {
            lit++;
...
void LocalMapping::CreateNewMapPoints()
{
    ...
    for(size_t i=0; i<vpNeighKFs.size(); i++)
    {
        ...
        for(int ikp=0; ikp<nmatches; ikp++)
        {
            ...
            const bool fallbackSource = !KeyFrameSemanticSourceIsStatic(mpCurrentKeyFrame, idx1) ||
                                         !KeyFrameSemanticSourceIsStatic(pKF2, idx2);
            if(mpTracker && mpTracker->GetSemanticConfig().enableMapPointPromotion && fallbackSource)
            {
                ...
                const bool secondValid = ComputeKeyFrameReprojectionError(pKF2, idx2, x3D, secondError);
                if(secondValid && (!reprojectionValid || secondError < reprojectionError))
                {
